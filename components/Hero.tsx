...
export function Hero() {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-white selection:bg-black selection:text-white font-mono">
      ...
      <div className="absolute top-0 flex w-full justify-between px-4 py-3 md:px-6 md:py-4 z-20 border-b-2 border-black/10 bg-white/80 backdrop-blur-md">
        <div className="flex items-center gap-3 md:gap-6">
          <div className="flex flex-col">
            ...
            <span className="text-[7px] md:text-[8px] text-zinc-400 uppercase font-bold">DXB_NODE // 14ms</span>
          </div>
          <div className="h-6 w-[1.5px] bg-black/20 hidden sm:block" />
          <div className="hidden lg:flex gap-4">
             <div className="flex flex-col gap-1">
                <span className="text-[7px] font-black uppercase text-zinc-400">Global_HP</span>
             ...
             <div className="flex flex-col gap-1">
                <span className="text-[7px] font-black uppercase text-zinc-400">Global_MP</span>
                <div className="h-2.5 w-24 bg-zinc-100 relative overflow-hidden border border-black/5">
        ...
        <div className="text-right flex flex-col items-end">
          ...
          <p className="text-[7px] md:text-[8px] text-green-600 font-bold animate-pulse tracking-widest flex items-center gap-1 uppercase">
            <span className="h-1 w-1 bg-green-500 rounded-full" /> LIVE
          </p>
        </div>
      </div>

      <section className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl flex-col items-center justify-center px-4 md:px-6 pt-24 pb-12">
        
        <div className="relative flex flex-col lg:flex-row items-center justify-between w-full gap-8 md:gap-16">
          
          {/* --- LEFT SIDE: SYSTEM LOGS (Hidden on small mobile, visible on tablet+) --- */}
          <div className="hidden md:flex xl:flex flex-col gap-8 w-full lg:w-48 opacity-60">
            <div className="space-y-2">
              <span className="text-[9px] font-black border-b border-black w-full block pb-1 tracking-widest uppercase text-black">System_Logs</span>
              <div className="space-y-0.5 font-mono text-[8px] text-zinc-500 italic uppercase">
                <p>&gt; AUTH_SUCCESS</p>
                <p>&gt; DUBAI_NODE_SYNCED</p>
                <p className="text-black font-bold animate-pulse">&gt; LISTENING...</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative flex flex-col items-center w-full">
            ...
            <div className="relative">
               {/* Target Reticle Decorators - Responsive Sizing */}
               ...
               <h1 className="text-center text-5xl sm:text-7xl md:text-8xl lg:text-[10rem] xl:text-[12rem] font-black uppercase tracking-tighter leading-[0.8] mb-4 md:mb-6">
                 Brian <br />
                 <span className="text-zinc-200 relative inline-block">
                   ...
                   <span className="absolute -right-20 top-1/2 hidden 2xl:block text-[8px] text-zinc-400 font-mono text-left opacity-40 uppercase">
                      Profile: {"{"} Dev_Mode: "Minimalist" {"}"}
                   </span>
                 </span>
               </h1>
            </div>

            {/* Hero Stats Matrix - Refined for Mobile padding */}
            <div className="mt-2 md:mt-4 flex items-center gap-3 md:gap-6 border-y-2 border-black/5 py-3 md:py-4 w-full justify-center bg-white/40 backdrop-blur-sm px-4">
               ...
               <div className="text-center">
                  <p className="text-[8px] text-zinc-400 font-black uppercase mb-0.5">Experience</p>
                  <p className="text-sm md:text-xl font-black italic underline decoration-2 decoration-black underline-offset-4 whitespace-nowrap">7+ MONTHS</p>
               </div>
               <div className="h-8 w-[1.5px] bg-black/10" />
               <div className="text-center">
                  <p className="text-[8px] text-zinc-400 font-black uppercase mb-0.5">Class</p>
                  <p className="text-sm md:text-xl font-black tracking-widest">DEVELOPER</p>
               </div>
            </div>

            <p className="mt-6 md:mt-8 max-w-[280px] md:max-w-md text-center text-xs font-bold text-zinc-500 uppercase tracking-tight leading-tight italic">
              "Building high-performance digital experiences. Focused on minimalist design and clean code."
            </p>

            {/* Interactive Buttons - Stack on mobile */}
            <div className="mt-8 md:mt-10 flex flex-col sm:flex-row gap-3 md:gap-5 w-full max-w-sm md:max-w-lg">
              <motion.button 
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex-1 bg-black text-white p-4 md:p-5 text-[10px] font-black uppercase tracking-[0.3em] border-2 border-black transition-all"
              >
                Initiate_Projects
              </motion.button>
              <motion.button 
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex-1 border-2 border-black bg-white p-4 md:p-5 text-[10px] font-black uppercase tracking-[0.3em] text-black transition-all"
              >
                Establish_Uplink
              </motion.button>
            </div>
          ...
          <div className="flex flex-col items-center relative mt-8 lg:mt-0">
            {/* Rotating HUD - Smaller on mobile */}
            <motion.div 
              animate={{ rotate: 360 }}
            ...
            <div className="relative z-10">
              <div className="absolute -top-3 -right-3 bg-black text-white px-2 py-0.5 text-[8px] font-black z-20">
                LVL_99
              </div>
              
              <motion.div 
                whileHover={{ rotate: [0, -1, 1, 0], transition: { repeat: Infinity, duration: 0.2 } }}
                className="bg-white border-2 md:border-4 border-black p-6 md:p-10 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] md:shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden"
              >
            ...
            <div className="mt-6 md:mt-10 text-center space-y-1 md:space-y-2">
               <span className="text-[8px] md:text-[10px] font-black tracking-[0.3em] text-zinc-400 uppercase">Avatar: Active</span>
               <div className="flex gap-1 justify-center">
                  <div className="h-1 w-4 md:w-6 bg-black" />
                  <div className="h-1 w-4 md:w-6 bg-black" />
                  <div className="h-1 w-4 md:w-6 bg-zinc-200" />
               </div>
            </div>
          </div>
        ...
        >
          {[...Array(6)].map((_, i) => (
            <span key={i} className="flex items-center gap-4 md:gap-6">
              <span className="text-green-600">● SYSTEM_NOMINAL</span>
              <span className="text-zinc-300">//</span>
              <span>7+ MONTHS_EXP</span>
              <span className="text-zinc-300">//</span>
              <span>BUILD: NEXTJS_V15</span>
              <span className="text-zinc-300">//</span>
              <span>LOC: DUBAI_UAE</span>
              <span className="text-zinc-300">//</span>
            </span>
          ))}
        </motion.div>
